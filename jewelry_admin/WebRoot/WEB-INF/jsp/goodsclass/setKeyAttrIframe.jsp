...
<body>
	<input type="hidden" id="attr_id" value="keySetting_${catAttr.attr_id}">
	<form class="layui-form" style="padding:20px;" action=""
		id="keySetting_'+selects[i].id+'">
    ...
    $(document).ready(function () {
        layui.use('form', function () {
            ...
            form.on('submit(submit)', function (data) {
                ...
                $(parent.document).find("#" + attr_id + " input.key_option_type").val(option_type);
                ...
                $(parent.document).find("#" + attr_id + " input.key_option_type").next().html(option_type_name);
                $(parent.document).find("#" + attr_id + " input.key_is_index").val(is_index);
                $(parent.document).find("#" + attr_id + " input.key_is_index").next().html(is_index == 1 ? '<span style="color:green">是</span>' : '<span style="color:red">否</span>');
                $(parent.document).find("#" + attr_id + " input.key_is_must").val(is_must);
                $(parent.document).find("#" + attr_id + " input.key_is_must").next().html(is_must == 1 ? '<span style="color:green">是</span>' : '<span style="color:red">否</span>');
                $(parent.document).find("#" + attr_id + " input.key_is_custom_value").val(is_custom_value);
                $(parent.document).find("#" + attr_id + " input.key_is_custom_value").next().html(is_custom_value == 1 ? '<span style="color:green">是</span>' : '<span style="color:red">否</span>');
                $(parent.document).find("#" + attr_id + " input.key_manual_fee").val(manual_fee);
                $(parent.document).find("#" + attr_id + " input.key_manual_fee").next().html(manual_fee);

                $($(parent.document).find(".key_attr_checked")).attr("option_type", option_type);
                $($(parent.document).find(".key_attr_checked")).attr("is_index", is_index);
                $($(parent.document).find(".key_attr_checked")).attr("is_must", is_must);
                $($(parent.document).find(".key_attr_checked")).attr("is_custom_value", is_custom_value);
