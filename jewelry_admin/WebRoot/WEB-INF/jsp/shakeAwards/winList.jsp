...
<html>
  ...
  <body>
  <form action="/admin/shakeawards/winList" method="post" >
	  <div class="list_oper_div">
		  <input class="btn return" type="button" value="返回奖项设置列表"
				 onclick="goInfo('/admin/shakeawards/list',${shake_id})" />
	  </div>
  	  <div class="searchDiv">
  	  		<table class="searchTable">
				<input type="hidden" name="shake_id" value="${shake_id}">
				<input type="hidden" name="awards_id" value="${awards_id}">
  	  			<tr>
					<td>中奖人账户:</td>
					<td><input type="text" name="login_name_vague_s" value="${login_name_vague_s}"/></td>
		  	  		<td>
		  	  			 <input class="btn ol_colorbtn ol_greenbtn" type="button" value="查询" onclick="searchInfo('/admin/shakeawards/winList');"/>
			  	  		 <input class="btn ol_colorbtn ol_bredbtn" type="button" value="清空" onclick="clearSearch('/admin/shakeawards/winList');"/>
		  	  		</td>
	  	  		</tr>
	  	  	</table>
  	  </div>	
