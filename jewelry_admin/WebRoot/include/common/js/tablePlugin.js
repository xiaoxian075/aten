...
(function($) {  
     ...
     function showData(obj,opts,jsonData){     	
      ...
      var pageMethod={
		 ...
		 _tableUI:function(obj,opts){		 		
		 		$(obj).find("table").find("tr:even").css("background",opts.tableEven);
		 		$(obj).find("table").find("tr:odd").css("background",opts.tableOdd); 
		 		$(obj).find("table").find("tr:first").css("background",opts.tableTrFirst);//设置标题列颜色		 		
		 },
		 		...
		 		$(obj).find("table").find("tr:gt(0)").each(function(){
					  ...
					  $(this).hover(
						  function () {
						      ...
						      $(this).css("background",opts.hoverColor);
						  },
						  function () {
	 ...
	 function  createTable(obj,opts,jsonData){	
	 		  //添加列表头
	 		  if(opts.tabletitle!=""){
	 		  	$(obj).append("<div class='tabletitle'>"+opts.tabletitle+"</div>");	
	 		  }
	          //表格数据的读取与拼串
		      var tableStr="";
		      //获取容器ID
		      var contain=$(obj).attr("id");
		      tableStr+="<table class='contentTable'><tr class='th_title'>";
		      /**生成表格头部标题开始**/
		      for(var i=0;i<opts.columnModel.length;i++){
