...
//            initTestOrProduction();
    ...
    public void initConstantsParameter(){
        try{
            CommConstant.filePath = CommDictList.getDictVal("static_constant","file_path");
            CommConstant.testOrPoduction =CommDictList.getDictVal("static_constant","test_production");
            CommConstant.supplierNumber =CommDictList.getDictVal("static_constant","supplier_number");
            List<DictTable> list = CommConstant.dictMapList.get("static_constant");
            if(list.size()>0) {
                for (DictTable vo : list) {
                    CommConstant.dictConstantMap.put(vo.getDictMc(),vo.getDictBh());
                }
            }
        }catch (Exception e ){
            e.printStackTrace();
