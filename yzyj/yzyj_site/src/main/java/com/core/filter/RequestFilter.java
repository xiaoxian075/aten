    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        if(request.getRequestURI().indexOf("/admin/")!=-1 && checkExistence(url,arrKey) == 1) {
            ...
            if(checkLink(request) == 0){
                AppFunUtil.getReturnOutStr(response,"{\"message\":\"您无权限访问 , 请与管理员联系!\",\"statusCode\":0,\"success\":false,\"data\":\"\"}","UTF-8");
                return ;
            }
        }
...
//            }
            //进行模块操作验证
            if(handleModule.indexOf(MD5Util.encodeByMD5Base64(url+"de0#O32.56+/*DMF")) != -1){
                // 验证成功
                state = 1;
            }else{
                System.out.println("地址未配置:"+url);
                state = 0;
            }
        }catch (Exception e ){
            e.printStackTrace();
