...
<div class="modal-content" id="brand-content">
    ...
    <div class="modal-body">
        <form class="form-horizontal" role="form" id="editForm" method="POST" enctype="multipart/form-data" action="<%=basePath%>rest/admin/advert/edit">
            <input  name="id" id="id" value="${info.id}"  type="hidden"/>
            <input  name="path" id="path" value="${info.path}"  type="hidden"/>
            <div class="form-group" id="mcDiv">
                <div class="group">
                    <label class="col-sm-3 control-label">广告名称</label>
                    <div class="col-sm-9">
                        <input class="form-control" name="name" maxlength="50" rows="3" value="${info.name}"/>
                    </div>
                ...
                <div class="group">
                    ...
                    <div class="col-sm-9">
                        <select class="form-control" name="type">
                            ...
                            <c:forEach items="${advertType}" var="ls">
                ...
                <div class="group">
                    ...
                    <div class="col-sm-9">
                        <select class="form-control" name="category">
                            <option value="">请选择</option>
                            <c:forEach items="${advertCategory}" var="ls">
                                <option value="${ls.dictBh}" <c:if test="${info.category eq ls.dictBh}"> selected</c:if>>${ls.dictMc}</option>
                            </c:forEach>
                        </select>
                    </div>
                </div>
            ...
            <div class="form-group" >
                <div class="group">
                    ...
                    <div class="col-sm-9">
                        <input class="form-control" name="url" maxlength="255" rows="3" value="${info.url}" onblur="checkmc();"/>
                    </div>
                </div>
            </div>
            <c:if test="${not empty info.id}">
                <div class="form-group">
                    ...
                    <div class="col-sm-3" style="width:80px;width:50px">
                        <c:if test="${not empty info.path}">
                            <img src="${path}${info.path}" style="height:100%;width:100%"/>
                        </c:if>
                    </div>
                </div>
            </c:if>
            <div class="form-group">
                <div class="group">
                    ...
                    <div class="col-sm-9">
                        <input id="file1" type="file" name="file1" class="file-loading"><p style="color:red;font-size:16px;">(图片规格：1200px*500px)</p>
                    </div>
                </div>
            </div>
            <div class="form-group">
        ...
        if (flag) {
            $.ajaxFileUpload({
                ...
                data: {//加入的文本参数
                    ...
                    "advertId": $("select[name='advertId']").val(),
