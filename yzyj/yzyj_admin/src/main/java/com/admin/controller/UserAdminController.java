            ...
            if (authUserInfo != null) {
                if ("1".equals(authUserInfo.getStatus())) {
                    if (authUserInfo.getPassword().equals(password)) {
                        ...
                        request.getSession().removeAttribute("agencyPerson");
                        if(!StringUtils.isEmpty(authUserInfo.getAgentUnique())){
                            AgencyPerson agencyPerson = userService.getAgencyPerson(authUserInfo.getAgentUnique());
                            if(agencyPerson.getStatus() == 0){
                                model.addFlashAttribute("error", "代理人信息已被禁用");
                                return "redirect:/rest/admin/login";
                            }else{
                                request.getSession().setAttribute("agencyPerson", agencyPerson);
                            }
                        }
