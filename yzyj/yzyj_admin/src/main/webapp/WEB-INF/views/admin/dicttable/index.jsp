    ...
    <div class="row">
        <div class="col-xs-12">
            <div class="dataTables_length" id="dataTables-example_length">
                <label>
                    <input type="search" class="form-control input-sm" placeholder="字典名称" aria-controls="dataTables-example" name="queryBody" id="queryBody" value=""/>
                </label>
                <button id="message-query" type="button" class="btn btn-sm btn-success" ><i class="icon-search"></i> 查询 </button>

                <a href="#" class="btn btn-sm btn-success" id="init"><i  class="icon-plus"></i> 初始字典</a>
                <a href="#" class="btn btn-sm btn-success" id="add"><i  class="icon-plus"></i> 新增字典</a>
            </div>
        </div>
    </div>
</div>
<div class="row">
    <div class="col-xs-12">
        <h4><span style="color: red">注意：添加或者修改操作之后一定要初始化</span></h4>
    </div>
</div>
...
<script>
    $(function () {
        ...
        var table = $.dicttable.initTables("user-table", null);
    ...
    $("#init").click(function(){
        $.ajax({
            url: "<%=basePath%>rest/admin/dicttable/init",
            type: "post",
            dataType: "json",
    ...
    function oneInit(id) {
        $.ajax({
            type: "post",
            dataType: "json",
            url: "<%=basePath%>rest/admin/dicttable/oneInit",
            data:{id:id},
            success: function (json) {
                alert(json.message);
            },
    ...
    function del(id){
        if (confirm("确定要删除数据吗?删除后将无法还原。")) {
            $.ajax({
                url: "<%=basePath%>rest/admin/dicttable/delete",
                data: {id:id},
                type: "post",
                dataType: "json",
                success: function (json) {
