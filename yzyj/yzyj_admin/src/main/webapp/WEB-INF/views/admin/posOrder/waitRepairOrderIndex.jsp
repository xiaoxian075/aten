...
<script>
    $(function () {
        ...
        var table = $.waitRepairOrderList.initTables("user-table", null);
        $('#viewdetail').on('hide.bs.modal', function () {
            table.ajax.reload();
        })
    });
    function pass(id,orderNumber){
        var r = confirm("确认要通过?");
        if (r) {
            $.ajax({
                type: "POST",
                url: "<%=basePath%>rest/admin/posOrder/pass",
                traditional: true,
    ...
    function reject(id,orderNumber){
        var r = confirm("确认要驳回?");
        if (r) {
            $.ajax({
                type: "POST",
                url: "<%=basePath%>rest/admin/posOrder/reject",
                traditional: true,
                data: {id:id,orderNumber: orderNumber},
                dataType: "json",
                cache: false,
