...
<script>
    $(function () {
        ...
        var table = $.waitTyjList.initTables("user-table", null);
        $('#viewdetail').on('hide.bs.modal', function () {
            table.ajax.reload();
        })
    });

    function passTyj(deviceUnique,id){
        var r = confirm("确认通过设备退押金审批?");
        if (r) {
            $.ajax({
                type: "POST",
                url: "<%=basePath%>rest/admin/deviceList/passTyj",
                traditional: true,
                data: {deviceUnique: deviceUnique,id:id},
                dataType: "json",
                cache: false,
    ...
    function rejectTyj(deviceUnique,id){
        var r = confirm("确认驳回设备退押金审批?");
        if (r) {
            $.ajax({
                type: "POST",
                url: "<%=basePath%>rest/admin/deviceList/rejectTyj",
                traditional: true,
                data: {deviceUnique: deviceUnique,id:id},
                dataType: "json",
                cache: false,
