...
<script>
    $(function () {
        ...
        var table = $.restCodeList.initTables("user-table", null);
        $('#viewdetail').on('hide.bs.modal', function () {
            table.ajax.reload();
        })
    });

    function update(machineCode,unique){
        var r = confirm("确认要重新激活设备?");
        if (r) {
            $.ajax({
                type: "POST",
                url: "<%=basePath%>rest/admin/deviceList/editDeviceListState",
                traditional: true,
                data: {machineCode: machineCode,deviceUnique: unique},
                dataType: "json",
                cache: false,
